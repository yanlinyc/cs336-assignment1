...
class NumpySnapshot:
    ...
    def get_np_arrays(self, test_name: str) -> dict[str, np.ndarray]:
        """
        Load the snapshot and return the arrays as a dictionary.

        Args:
            test_name: The name of the test (used for the snapshot file)

        Returns:
            Dictionary of NumPy arrays from the snapshot
        """
        snapshot_path = self._get_snapshot_path(test_name)
        if not snapshot_path.exists():
            raise FileNotFoundError(f"Snapshot file {snapshot_path} does not exist.")

        return dict(np.load(snapshot_path))
