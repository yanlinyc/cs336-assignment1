...
def get_batch(
    ...
    context_length: int,
    device: str,
    fixed_starts: list[int] | None = None,
    ...
    if fixed_starts is not None:
        assert len(fixed_starts) == batch_size, (
            f"Expected {batch_size} fixed starts, but got {len(fixed_starts)}."
        )
        starts = np.array(fixed_starts, dtype=np.int64)
    else:
